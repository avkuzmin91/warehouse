...
| `products` | товары и варианты: SKU (+`sku_pending`), штрих-коды (несколько на вариант, `product_barcodes` с `variant_id`; этикетки: загруженный файл — `product_barcode_files`, печатная форма Code 128 рисуется по цифрам на каждый запрос и не хранится — `POST /products/barcode-labels`, `barcode_svg` в `utils.py`; кандидат выбирается магазином строки (`product_barcodes.store_id`), а коды разных кабинетов требуют явного выбора — он живёт на строке документа, `shipment_lines.label_barcode`), фото; смена цвета/размера варианта — журнал `variant_identity_changes`; массовая загрузка из Excel по клиенту — `/products/bulk-import/*` (строка файла = вариант, справочники строгие, превью `product_import_batches` → применение) |
...
| `containers` | короба задачи «Упаковка с ТСД»: `containers` + журнал `container_ops`, этикетки QR (`wms:box:`); здесь же **все перемещения по складу** — `POST /containers/place` («откуда → что → куда»: развозка упакованного в зону отгрузки, переезд короба, перенос товара с полки, в короб, из короба и между коробами), изъятие позиции из размещённого короба, свод «что лежит в коробах» для остатков (`GET /containers/holdings`) |
...
| `timesheet` | табель (неделя Сб→Пт), ставки effective-dated, выплаты; переработка — свыше 12 ч **на смене** (порог по gross, обед вычитается из базовой части): первые 4 ч ×1.3, дальше ×1.5, с даты `TIMESHEET_OVERTIME_EFFECTIVE_FROM`. Заработок за день считать только через `entry_earned` — тем же расчётом живут пятничный расчёт и дневные начисления P&L. Расчёт может быть частичным: состояние строки (`PAYOUT_STATUS_*`) выводится из денег (`to_pay`), а не из наличия платежа, остаток можно доплатить; факт часов запирается только при полном закрытии недели (`locked_employee_ids`) |
...
| `marketplaces` | интеграция с маркетплейсами (FBS Ozon/WB): аккаунты `mp_accounts`, товары/связки `mp_products`/`mp_product_links`, заказы `mp_orders`/`mp_order_lines`, журнал синка `mp_sync_log`, магазины клиентов `client_stores`; поставки `mp_supplies`/`mp_supply_orders`/`mp_supply_ops`, журнал сборки `mp_supply_picks`, журнал упаковки по заказам `mp_supply_packs` (ШК/КИЗ → заказ), грузовые места `mp_cargo_units`/`mp_cargo_unit_orders`; HTTP-клиенты площадок — `clients.py` (четвёртый файл модуля, осознанное исключение): чтение заказов/карточек + пишущий контур (сборка отправления и КИЗ Ozon, этикетки, поставки/короба/deliver WB) |
...
- Корзина `INV_OP_PICKED` («Собрано под МП») намеренно вне пулов доступности. В неё уходит собранное сборщиком по FBS-поставке (`packed@место → picked`), обратно — только откатом скана; со склада списывается одним движением `picked → shipped`, когда поставка передана площадке.
- Пулы доступности: пул отгрузки — `ready`+`packed` (`DISPATCH_ALLOW_SHIP_FROM_PACKED`), пул упаковки — `storage`, пул сборки FBS — только `packed` (`MP_SUPPLY_PICK_OP`: под заказ площадки берётся товар, прошедший стол упаковки; готовность заказа и лист подбора считаются по нему же). Задача «Упаковка с ТСД» пишет скан в короб и «без короба» в ту же корзину `packed`, что и обычная упаковка (с осью короба или без), поэтому собранное доступно отгрузке сразу; развозка коробов переводит его `packed → ready@место`. Отдельной корзины «ждёт размещения» нет (бывший `boxed` слит с `packed` в 0110).
- Третья ось — **короб**: `zone_relocations.from_container_id`/`to_container_id`. Содержимое короба = нетто журнала по этой оси (отдельной таблицы содержимого нет), поэтому перенос короба обязан ставить обе стороны, а собранное без короба отличается пустой осью. Короб живёт в любой корзине: `packed` — у стола, `ready` — развезён, `storage` — переехал на хранение вручную; развозка и переезд не меняют содержимого, только корзину/место. Ручные операции с остатком, лежащим в коробе, запрещены (гейт `_ensure_not_boxed` в `balances/service.py` — считает свободный остаток места, россыпь рядом с коробом двигать можно): короб едет целиком через `POST /containers/place` (или `/containers/{id}/move`), отдельная позиция выходит из короба только через ось короба — тем же `/containers/place` с `source.kind=container` (на полку или в другой короб) либо `POST /containers/{id}/items/remove` (остаётся в том же месте россыпью).
...
- **Задача склада — два типа (`shipment_docs.task_kind`):** `packing` «Упаковка под отгрузку» (количества вносятся числом за столом) и `putaway` «Упаковка с ТСД» (поштучный скан в короба). Результат один — терминал `packed` «Упакован», товар в пуле отгрузки, дальше его возит dispatch. Маршрут задачи с ТСД: `draft → packing → on_packing → packed` (без `relocating`). Процесс расщеплён на две фазы, и **вторая фаза документу не принадлежит**:
  - **Сборка** (документ, `on_packing`): скан короба → поштучный скан товара в него (короб — просто тара: и годный, и брак) → закрытие короба. Что в короб не идёт (габарит, особые причины) — скан «без короба» (`POST /shipments/{id}/putaway/aside`). Каждый скан — запись упаковки `packing → packed` (объём, дата, заработок). Ячейку в этой фазе никто не сканирует. Конец фазы — `POST /shipments/{id}/finish-collecting` → `packed` (гейт: набранных незакрытых коробов не осталось; нерешённый пул со стола возвращается на хранение). Возврат на упаковку (`return-to-packing`) для задачи с ТСД пока закрыт гейтом: откат раскладки не знает оси короба.
  - **Развозка** (вне документа, `POST /containers/place`): кладовщик берёт пачку закрытых коробов и упакованной россыпи, везёт к стеллажу и сканирует их, затем место — любое активное место справочника, не обязательно адресная ячейка; в учёте это `packed → ready@место` (зеркало «Готово к рейсу» у обычной упаковки). Тем же действием переезжает уже размещённый короб. **Статусы задач развозка не двигает**: задача закончилась на сборке, а одна ходка тележки увозит объекты разных задач и может увезти лишь часть. Очередь развозки живёт в коробах, а не в документах — `GET /containers/pending-placement` (закрытые короба + упакованное без короба **только задач с ТСД** — «Упаковано» обычной задачи раскладывает `finish_relocation` внутри документа, `since` = самый старый объект у стола); из неё же строится одна общая карточка «Развезти по местам» в «Моих задачах» (`kind=boxes_place`, без `doc_id`) и блок «Ждут развозки» на странице `/inventory/boxes`. След «куда уехало» дописывается в журнал задачи (`shipment_ops`) и после её закрытия.
  - Обе фазы делают кладовщик и начальник смены (`SHIPMENT_PUTAWAY_ROLES`, те же права, что у ручных операций с остатками).
- **Перемещения по складу — один эндпоинт `POST /containers/place`** («откуда → что → куда»: `source` = зона упаковки `collected` / место `location` / размещённый короб `container`, `target` = место / размещённый короб; `zone_id` — старая форма приёмника-места). Режим не выбирается: закрытый короб размещается, размещённый переезжает, товар едет со стола (упакованное без короба задачи с ТСД), с полки (`from_zone_id` = «взял отсюда»; россыпь `storage` и `ready`, корзина сохраняется) или из короба — на полку или в размещённый короб. Названный источник **сверяется**: короб, который числится в другом месте, — ошибка, не правка учёта; без `source` источник ищется сам и при неоднозначности отдаётся ошибка. Позиция задаётся ШК (ТСД) либо вариантом (веб — там сканера нет). **Размещённый короб — обычная тара**: в него докладывают и из него изымают (однородность по качеству сохраняется), неразмещённые короба приёмником не бывают (их состав меняет задача сборки), короб в короб не вкладывается. На ТСД это экран «Перемещение»: три шага с последовательной разблокировкой, товар поштучным сканом, в учёт пишется только по «Подтвердить перемещение». Ручные точечные операции с остатком (перемещение, качество, списание, массовая консолидация) остаются в `balances` — там же процессные бакеты (`packing`/`packed`/`ready`), которые ТСД не трогает.
...
- **FBS-поставки (`mp_supplies`):** `checking → picking → packing → handover → done`, плюс `cancelled` и боковой `correcting` («Корректировка», 0116) — только из `checking` и обратно. Статус `draft` («Создание») — легаси в БД: состав выбирают **до** заведения на отдельной странице `/marketplaces/supplies/new?account=` (не модалка; «Закрыть» ничего не сохраняет), поэтому поставка создаётся сразу на «Проверке» (`checking_at` ставится при создании). На «Проверке» состав read-only; перевыбор — действие «Скорректировать» (`POST /supplies/{id}/correct` → `correcting`, тот же экран галочек, что при создании; `.../correct/apply` с `order_ids` применяет целиком и возвращает в `checking`, `.../correct/discard` — без изменений; пустой состав не применяется). **Передача площадке** — `POST /supplies/{id}/transfer` («Передать поставку WB / Ozon», гейт тот же, что у передачи в сборку: состав не пуст, блокеров нет): WB — заводится поставка продавца (`external_supply_id`) и в неё уходят все задания (`mp_shipped_at`), Ozon — только фиксация состава (сущности «поставка» до сборки отправлений у него нет); отметка `mp_transferred_at`. После неё состав не правится (ни `PUT /orders`, ни корректировка), поставка **не аннулируется**, только с неё доступна лента этикеток, и только переданная уходит в сборку (`advance` из `checking` иначе отдаёт блокер). Фаза `picking` — работа **сборщика** (роль `picker`, `MP_SUPPLY_PICK_ROLES`; те же права у кладовщика и начальника склада — на малом складе выделенного человека нет). Очередь вытягивающая: `POST /marketplaces/supplies/claim-next` захватывает следующую по отсечке поставку атомарно (`FOR UPDATE SKIP LOCKED`), взятая пропадает из чужих очередей. Сборка идёт сканом на ТСД — место → короб (если товар в коробе) → ШК товара поштучно (`POST /supplies/{id}/picks`, откат — `.../picks/{pick_id}/undo`), лист подбора свёрнут **по вариантам** и отсортирован по маршруту обхода (`GET /supplies/{id}/pick-view`). **Сборка закрывается только полностью собранным составом**: недобора нет, тупик «товара физически нет» развязывает менеджер, снимая заказ (`POST /supplies/{id}/orders/{order_id}/drop`) — тогда потребность уменьшается и `finish-picking` проходит. Уход со сборки закрывает приём (дозагрузки больше нет).
  - **Свободный пул — владелец заказа** (0113). Синк только приносит заказы: поставку заводит менеджер (`POST /marketplaces/supplies`, кнопка «Новая поставка»), и поставок у кабинета столько, на сколько он решил поделить поток — **поставка = отгрузка FBS**. Автораскладки по волнам больше нет, `ux_mp_supplies_open_wave` снят; вместо него инвариант «заказ ровно в одной активной поставке» держит частичный индекс `ux_mp_supply_orders_active`. Заведение идёт от состава, а не от документа: кнопка «Новая поставка» открывает страницу создания с выбором заказов из пула (`GET /marketplaces/supplies/pool?account_id=`), поставка создаётся уже с составом — **пустую поставку завести нельзя** (400), иначе доска зарастает пустышками. Дальше состав перевыбирается тем же экраном через «Скорректировать» (кандидаты — `GET /supplies/{id}/candidates`; и пул, и кандидаты приходят разобранными: состав, ячейки, блокеры) и применяется целиком кнопкой шапки; `PUT /supplies/{id}/orders` остаётся для «Исключить проблемные» на «Проверке» до передачи площадке; снятый заказ возвращается в пул. Выбор при заведении и при корректировке — один экран (`OrderSelectionPanel` поверх `OrderPickTable`), порядок везде по дедлайну. **Отсечка `cutoff_at` — следствие состава** (самый ранний дедлайн, вниз до часа, `recompute_supply_cutoff`), поэтому одна поставка может смешивать волны; `intake_closes_at` едет за ней. Пока приём открыт, заказ из пула добирается в идущую сборку (`POST /supplies/{id}/dock`), после закрытия приёма состав не растёт. Страховка от «заказ сгорел в пуле» — только алярм: блок «Свободные заказы» на доске (`free_pool` в `/supplies/board`) с подсветкой просроченных и горящих (`MP_SUPPLY_POOL_ALARM_HOURS`).
  - **Упаковка (`packing`)** — станция упаковки (веб `/marketplaces/supplies/:id/pack`, ТСД «Упаковка»), роли `ensure_supply_work_access` = сборщик/склад + менеджер. Собранное раскладывается **по заказам** поштучным сканом ШК варианта или кода маркировки ЧЗ (`POST /supplies/{id}/orders/{order_id}/pack-scans`, журнал `mp_supply_packs`, откат `.../pack-scans/{pack_id}/undo`); КИЗ пишется в реестр `marking_codes` и привязывается к заказу, повторно тот же код в другой заказ не ложится. Гейт скана — стол: уложить можно не больше, чем собрано по варианту (`picked`), сток при упаковке не двигается. `POST .../orders/{order_id}/pack` закрывает укомплектованный заказ (`mp_orders.packed_at`) и сразу идёт на площадку (`push_order_to_marketplace`, две транзакции: факт укладки не зависит от ответа МП): Ozon — `exemplar/set` с КИЗ + `/v4/posting/fbs/ship` + этикетка PDF; WB — поставка `WB-GI` (`external_supply_id`, создаётся при первом заказе) + батч `PATCH /api/marketplace/v3/supplies/{id}/orders` (пер-заказного пути `/api/v3/.../orders/{order}` у площадки больше нет) + `sgtin` + стикер PNG. Файл этикетки — в `UPLOADS_DIR` (`label_url`), штрих-код стикера — `label_barcode` (Ozon — номер отправления); ошибка площадки — `mp_error`, повтор `POST .../push`. `unpack` — только пока `mp_shipped_at` пуст **у Ozon**: у WB отметка стоит и до упаковки (см. ленту этикеток), а перекладка коробки площадке не видна.
  - **Лента этикеток заранее** (`POST /supplies/{id}/labels`, `MP_SUPPLY_LABEL_STATUSES` = проверка/сборка/упаковка, кнопка «Получить этикетки» в карточке поставки) — **только WB и только после «Передать поставку WB»** (`mp_transferred_at`): стикер выдаётся заданию, лежащему в поставке продавца, и КИЗ уходит позже, до `deliver`. Пачка стикеров печатается лентой 58×40 (`openOrderLabelSheet`) и клеится на заказы по ходу работы. Ozon этикетку до сборки отправления не отдаёт, а сборке обязан предшествовать КИЗ со скана упаковки — там она и приходит. Вынуть задание из поставки WB площадка не даёт: снятый заказ — след в журнале и работа менеджера в кабинете. Гейт `finish-packing`: все заказы упакованы **и** с этикеткой, на столе ничего не осталось.
  - **Передача (`handover`)** — грузовые места `mp_cargo_units` (`GM-000001`, короб/палета, QR `wms:gm:<id>`, состав `mp_cargo_unit_orders`, заказ ровно в одном ГМ): `POST /supplies/{id}/cargo`, скан этикетки заказа (штрих-код стикера / номер) в активное ГМ `POST /cargo/{id}/orders`, закрытие `/close`, переоткрытие `/reopen` (пока нет `external_id`). Веб `/marketplaces/supplies/:id/cargo`, ТСД «Грузовые места» (скан QR ГМ → этикетки заказов → QR ГМ = закрыть). Гейт «Передана площадке» (`advance` менеджером): все упакованные заказы в **закрытых** ГМ. Перед сменой статуса `handover_supply_to_marketplace`: WB — короба `trbx` по ГМ (`external_id`), состав, `deliver`; Ozon — ничего. Затем `picked → shipped`. Задачи: `mp_supply_pick` / `mp_supply_pack` (сборщик, взятая поставка остаётся за ним) / `mp_supply_cargo` (склад). Привязка поставки к рейсу — не сделана (блок 3 схемы FBS).
...
- Один APK для всех ролей: раскладка табов по роли (`tabsForRole`) — кладовщик, начальник смены, начальник склада, менеджер, сборщик (у сборщика только «Мои задачи» и «Сборка»: очередь FBS-поставок и экран скана).
