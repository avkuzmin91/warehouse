...
| `marketplaces` | интеграция с маркетплейсами (FBS Ozon/WB): аккаунты `mp_accounts`, товары/связки `mp_products`/`mp_product_links`, заказы `mp_orders`/`mp_order_lines`, журнал синка `mp_sync_log`, магазины клиентов `client_stores`; поставки `mp_supplies`/`mp_supply_orders`/`mp_supply_ops` и журнал сборки `mp_supply_picks`; HTTP-клиенты площадок — `clients.py` (четвёртый файл модуля, осознанное исключение) |
...
- Корзина `INV_OP_PICKED` («Собрано под МП») намеренно вне пулов доступности. В неё уходит собранное сборщиком по FBS-поставке (`storage@место → picked`), обратно — только откатом скана; со склада списывается одним движением `picked → shipped`, когда поставка передана площадке.
- Пулы доступности: пул отгрузки — `ready`+`packed` (`DISPATCH_ALLOW_SHIP_FROM_PACKED`), пул упаковки — `storage`. Задача «Упаковка с ТСД» пишет скан в короб и «без короба» в ту же корзину `packed`, что и обычная упаковка (с осью короба или без), поэтому собранное доступно отгрузке сразу; развозка коробов переводит его `packed → ready@место`. Отдельной корзины «ждёт размещения» нет (бывший `boxed` слит с `packed` в 0110).
