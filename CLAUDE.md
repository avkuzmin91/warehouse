...
| `containers` | короба задачи «Размещение по ячейкам»: `containers` + журнал `container_ops`, этикетки QR (`wms:box:`); здесь же **все перемещения по складу** — `POST /containers/place` («откуда → что → куда»: развозка собранного, переезд короба, перенос товара с полки, в короб, из короба и между коробами), изъятие позиции из размещённого короба, свод «что лежит в коробах» для остатков (`GET /containers/holdings`) |
...
- Корзина `INV_OP_PICKED` («Собрано под МП») — то же правило, что у `boxed`: вне пулов доступности. В неё уходит собранное сборщиком по FBS-поставке (`storage@место → picked`), обратно — только откатом скана; со склада списывается одним движением `picked → shipped`, когда поставка передана площадке.
- Корзина `INV_OP_BOXED` («Ждёт размещения») намеренно НЕ входит ни в один пул доступности: пул отгрузки — `ready`+`packed` (`DISPATCH_ALLOW_SHIP_FROM_PACKED`), пул упаковки — `storage`. В ней лежит всё собранное задачей размещения — и в коробе, и мимо короба (габарит, брак), — пока оно не уехало в место хранения (`boxed → storage@место`).
- Третья ось — **короб**: `zone_relocations.from_container_id`/`to_container_id`. Содержимое короба = нетто журнала по этой оси (отдельной таблицы содержимого нет), поэтому перенос короба обязан ставить обе стороны, а собранное мимо короба отличается пустой осью. Ручные операции с остатком, лежащим в коробе, запрещены (гейт `_ensure_not_boxed` в `balances/service.py` — считает свободный остаток места, россыпь рядом с коробом двигать можно): короб едет целиком через `POST /containers/place` (или `/containers/{id}/move`), отдельная позиция выходит из короба только через ось короба — тем же `/containers/place` с `source.kind=container` (на полку или в другой короб) либо `POST /containers/{id}/items/remove` (остаётся в том же месте россыпью).
