...
| `containers` | короба задачи «Размещение по ячейкам»: `containers` + журнал `container_ops`, этикетки QR (`wms:box:`); здесь же **развозка по местам** — `POST /containers/place` (пачка коробов и россыпи → одно место, оно же перемещение уже размещённого короба) и изъятие позиции из размещённого короба |
