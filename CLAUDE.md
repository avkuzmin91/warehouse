...
- **Задача склада — два типа (`shipment_docs.task_kind`):** `packing` «Упаковка под отгрузку» (терминал `packed`, дальше товар возит dispatch) и `putaway` «Размещение по ячейкам» (терминал `placed`). Маршрут размещения: `draft → packing → on_packing → collected → placed`. Процесс расщеплён на две фазы:
  ...
  - **Развозка** (без документа, `POST /containers/place`): кладовщик берёт пачку закрытых коробов и собранной россыпи, везёт к стеллажу и сканирует их, затем место хранения — любое активное место справочника, не обязательно адресная ячейка. Тем же действием переезжает уже размещённый короб. Задача уходит в `placed` **сама** (`maybe_close_putaway_doc`), когда уехал её последний объект — отдельного «финиша» у развозки нет.
