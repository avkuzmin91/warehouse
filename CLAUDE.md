...
| `containers` | короба задачи «Размещение по ячейкам»: `containers` + журнал `container_ops`, этикетки QR (`wms:box:`); здесь же **все перемещения по складу** — `POST /containers/place` (пачка коробов и товара → одно место: развозка собранного, переезд короба, перенос товара с полки), изъятие позиции из размещённого короба, свод «что лежит в коробах» для остатков (`GET /containers/holdings`) |
...
- **Перемещения по складу — один эндпоинт `POST /containers/place`** («пачка объектов → одно место»). Режим не выбирается: закрытый короб размещается, размещённый переезжает, товар едет из корзины «ждёт размещения», а если её нет — переносится с полки (`from_zone_id` = «взял отсюда»; без него источник ищется сам и при неоднозначности отдаётся ошибка). Позиция задаётся ШК (ТСД) либо вариантом (веб — там сканера нет). Товар в коробе двигается только коробом: сначала `POST /containers/{id}/items/remove`. Ручные точечные операции с остатком (перемещение, качество, списание, массовая консолидация) остаются в `balances` — там же процессные бакеты (`packing`/`packed`/`ready`), которые ТСД не трогает.
