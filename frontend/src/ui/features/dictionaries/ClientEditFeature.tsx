...
export function ClientEditFeature({ id }: { id: string }) {
  ...
  useEffect(() => {
    getDictionaryItem('clients', id).then((c) => {
      setName(c.name)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      setSaving(true)
      setError('')
      await updateDictionaryItem('clients', id, { name: name.trim(), is_active: isActive })
  ...
  return (
    <DetailPage title="Редактировать клиента" backTo="/dictionaries/clients">
