...
import {
  EmpAvatar, fmtHours, fmtMoney, fmtOvertimeHours, calcDayHours, calcOvertimeHours,
  OVERTIME_THRESHOLD_HOURS, OVERTIME_TIER1_MULT, OVERTIME_TIER2_MULT,
} from './shared'
...
/** Строка разбивки заработка за день: часы (× коэффициент) → сумма. */
function BandRow({ hours, mult, rate }: { hours: number; mult?: number; rate: number | null }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, padding: '3px 0' }}>
      <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>
        <span className="mono">{fmtOvertimeHours(hours)}</span>
        {mult ? <span style={{ color: 'var(--c-warning)', fontWeight: 600 }}> ×{mult.toLocaleString('ru-RU')}</span> : ' базовых'}
      </span>
      {rate != null && (
        <span className="mono" style={{ fontSize: 12, fontWeight: 600 }}>{fmtMoney(hours * rate * (mult ?? 1))}</span>
      )}
    </div>
  )
}
...
export function DayCardDrawer({ employeeId, employeeName, workDate, today, onClose, onSaved }: Props) {
  ...
  const [ot1, ot2] = calcOvertimeHours(fs || null, fe || null, workDate, { endNextDay })
  const baseHours = Math.round((hours - ot1 - ot2) * 100) / 100
  const rate = detail?.rate_kopecks ?? null
  const earned = rate != null
    ? Math.round(rate * (baseHours + ot1 * OVERTIME_TIER1_MULT + ot2 * OVERTIME_TIER2_MULT))
    : null
  ...
  return (
    <Drawer open onClose={onClose} width={460} padded={false}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 18 }}>
        ...
        {!isFuture && (
          <div style={{ marginBottom: 16, borderRadius: 'var(--r-lg)', background: 'var(--c-bg-sunken)', border: '1px solid var(--c-border)' }}>
            ...
            {ot1 + ot2 > 0 && (
              <div style={{ padding: '10px 14px', borderTop: '1px solid var(--c-border)', background: 'var(--c-warning-bg)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  <Icon name="zap" size={13} style={{ color: 'var(--c-warning)' }} />
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-warning)' }}>
                    Переработка {fmtOvertimeHours(ot1 + ot2)}
                  </span>
                  <span style={{ fontSize: 11, color: 'var(--c-text-subtle)' }}>
                    сверх {OVERTIME_THRESHOLD_HOURS} ч на смене
                  </span>
                </div>
                <BandRow hours={baseHours} rate={rate} />
                {ot1 > 0 && <BandRow hours={ot1} mult={OVERTIME_TIER1_MULT} rate={rate} />}
                {ot2 > 0 && <BandRow hours={ot2} mult={OVERTIME_TIER2_MULT} rate={rate} />}
                {earned != null && (
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10, paddingTop: 5, marginTop: 3, borderTop: '1px solid var(--c-border)' }}>
                    <span style={{ fontSize: 12, color: 'var(--c-text-muted)' }}>Итого за день</span>
                    <span className="mono" style={{ fontSize: 13, fontWeight: 700 }}>{fmtMoney(earned)}</span>
                  </div>
                )}
              </div>
            )}
