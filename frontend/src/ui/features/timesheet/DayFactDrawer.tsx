...
import { EmpAvatar, fmtHours, fmtOvertimeHours, calcDayHours, calcOvertimeHours, OVERTIME_TIER1_MULT, OVERTIME_TIER2_MULT } from './shared'
...
export function DayFactDrawer({ date, dateLabel, rows, isFuture, onClose, onSaved, onPrevDay, onNextDay }: Props) {
  ...
  return (
    <Drawer open onClose={onClose} width={600} padded={false}>
        ...
        <>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            {rows.map((r) => {
              ...
              const [ot1, ot2] = s.mode === 'work'
                ? calcOvertimeHours(s.fs || null, s.fe || null, date, { endNextDay: s.nd })
                : [0, 0]
              ...
              return (
                ...
                >
                  ...
                  {/* Переработка видна сразу при вводе: и как факт для оплаты, и как ловушка для опечатки во времени. */}
                  {!r.locked && ot1 + ot2 > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 7, paddingLeft: 42, fontSize: 11.5, color: 'var(--c-warning)' }}>
                      ...
                      <span style={{ color: 'var(--c-text-subtle)' }}>
                        {ot1 > 0 && `${fmtOvertimeHours(ot1)} ×${OVERTIME_TIER1_MULT.toLocaleString('ru-RU')}`}
                        {ot1 > 0 && ot2 > 0 && ' + '}
                        {ot2 > 0 && `${fmtOvertimeHours(ot2)} ×${OVERTIME_TIER2_MULT.toLocaleString('ru-RU')}`}
                      </span>
                    </div>
                  )}
