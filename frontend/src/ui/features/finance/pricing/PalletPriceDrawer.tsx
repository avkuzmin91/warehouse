...
import { deleteClientPalletPrice, getClientPalletPrices, setClientPalletPrice } from '../../../../api/palletPricingApi'
import type { ClientPalletPriceDetail, PalletPriceHistoryEntry } from '../../../../api/palletPricingApi'
...
export function PalletPriceDrawer({ clientId, onClose, onSaved }: Props) {
  const toast = useToast()
  const confirm = useConfirm()
  const [detail, setDetail] = useState<ClientPalletPriceDetail | null>(null)
  ...
  const reload = useCallback(() => {
    return getClientPalletPrices(clientId)
      .then((d) => setDetail(d))
  ...
  useEffect(() => {
    let alive = true
    setLoading(true)
    getClientPalletPrices(clientId)
  ...
  const deleteEntry = useCallback(async (entry: PalletPriceHistoryEntry) => {
    ...
    if (!ok) return
    try {
      await deleteClientPalletPrice(clientId, entry.id)
  ...
  function save() {
    ...
    if (kop === null) { toast('Укажите стоимость палета', 'error'); return }
    setSaving(true)
    setClientPalletPrice(clientId, { price_kop: kop, effective_from: effFrom })
      .then(() => { toast('Цена палета сохранена', 'success'); onSaved() })
      .catch((e) => toast(e instanceof Error ? e.message : String(e), 'error'))
  ...
  return (
    <Drawer
      open
      onClose={onClose}
      title={detail ? detail.client_name : 'Стоимость палета'}
      subtitle="Цена за один палет"
      width={520}
      footer={
        <>
      ...
      {loading ? (
        ...
        <>
          <div className="card" style={{ padding: '10px 12px', marginBottom: 6 }}>
            <div style={{ fontSize: 11.5, color: 'var(--c-text-subtle)' }}>Цена палета сейчас</div>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--c-text-subtle)', margin: '14px 0 8px' }}>
            Новая цена (за один палет). Действует с указанной даты; если она в прошлом — применится и к более раннему.
          </div>

          <label style={{ display: 'block', marginTop: 12 }}>
            <div style={{ fontSize: 12, color: 'var(--c-text-subtle)', marginBottom: 5 }}>Стоимость палета, ₽</div>
            <input className="input sm" inputMode="decimal" placeholder="напр. 350,00"
          ...
          <HistoryBlock title="История цены палета" entries={detail?.history ?? []} onDelete={deleteEntry} />
        </>
      )}
    </Drawer>
  )
...
function HistoryBlock({ title, entries, onDelete }: { title: string; entries: PalletPriceHistoryEntry[]; onDelete: (entry: PalletPriceHistoryEntry) => void }) {
  return (
