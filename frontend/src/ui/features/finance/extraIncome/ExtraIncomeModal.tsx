...
export function ExtraIncomeModal({ entry, categories, onClose, onSaved }: {
  ...
  // При редактировании цена восстанавливается делением, если делится нацело до копейки;
  // иначе (легаси-запись без количества или с некруглой ценой) поле пустое.
  const [amount, setAmount] = useState(
    entry && entry.qty != null && entry.qty > 0 && entry.amount_kop % entry.qty === 0
      ? String(entry.amount_kop / entry.qty / 100)
      : '',
  )
  ...
  const totalKop = kopecks != null && qtyValid ? kopecks * qtyNum : null
  const problems = [
    ...(!entryDate ? ['Укажите дату'] : []),
    ...(!clientId ? ['Выберите клиента'] : []),
    ...(!categoryId ? ['Выберите вид работы'] : []),
    ...(!qtyValid ? ['Количество — целое число больше нуля'] : []),
    ...(kopecks == null || kopecks <= 0 ? ['Цена за единицу — число больше нуля'] : []),
    ...(!comment.trim() ? ['Напишите комментарий — что делали'] : []),
  ]
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div>
          <label style={labelStyle}>Цена за шт., ₽</label>
          <input className="input" inputMode="decimal" placeholder="например, 15" value={amount} disabled={locked}
        ...
        ) : legacyNoUnitPrice && !locked ? (
          <div style={{ gridColumn: '1 / -1', fontSize: 12.5, color: 'var(--c-text-muted)', marginTop: -4 }}>
            Запись сохранена суммой {formatMoneyKopecks(entry!.amount_kop)} — укажите количество и цену за штуку, итог пересчитается.
          </div>
        ) : null}
        <div style={{ gridColumn: '1 / -1' }}>
          <label style={labelStyle}>Комментарий — что делали</label>
