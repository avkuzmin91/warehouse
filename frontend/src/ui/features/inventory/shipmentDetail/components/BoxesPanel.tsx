...
/** Короба задачи с ТСД: что собрано и где оно теперь.
 *
...
export function BoxesPanel({ docId, doc, canEdit, collected = false, onDone }: Props) {
  ...
  async function handleFinish() {
    ...
    const body = [
      collected > 0
        ? `Собрано ${collected} шт из ${planTotal} шт — это упаковано и доступно отгрузке. Задача закроется: короба и товар без короба уйдут в очередь развозки — их развезёт кладовщик, когда повезёт ходку.`
