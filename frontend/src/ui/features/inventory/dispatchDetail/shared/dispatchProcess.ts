...
const DSP_STATUS_ORDER: DispatchStatus[] = ['draft', 'awaiting_trip', 'partially_shipped', 'shipped']
...
function getStepTimestamps(ops: DispatchOp[]): Partial<Record<DispatchStatus, string>> {
  ...
  for (const op of [...ops].reverse()) {
    ...
    if (op.op_type === 'advance' && !ts.awaiting_trip) ts.awaiting_trip = op.created_at
    if (op.op_type === 'ship') {
