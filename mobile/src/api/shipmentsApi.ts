...
export function undoShipmentBoxItem(
  docId: string, boxId: string, lineId: string, qty: number, requestId: string,
) {
...
export function releaseShipmentBox(docId: string, boxId: string, requestId: string) {
  return request<{ message: string }>(`/shipments/${docId}/boxes/${boxId}/release`, {
    method: 'POST',
    headers: requestIdHeaders(requestId),
  })
}
