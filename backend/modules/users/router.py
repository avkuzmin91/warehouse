...
def update_user_role(user_id: str, payload: RoleUpdateRequest, admin=Depends(_get_users_admin)):
    if payload.role not in ("user", "manager", "warehouse_manager", "shift_supervisor", "client"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Можно назначить роль: user, manager, warehouse_manager, shift_supervisor или client",
        )
    if user_id == admin["id"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
