...
def _logistics_income_by_day(
    connection, *, date_from: str, date_to: str, client_id: str | None,
) -> dict[str, int]:
    """День (факт рейса) → стоимость логистики для клиента, копейки.

    Берётся из `*.logistics_cost` (рубли) отгрузок и поступлений, привязанных к рейсу,
    по дню `arrived_at` рейса. Документ относим к самому раннему рейсу в окне."""
    ...
    for table, link_col in (("dispatch_docs", "dispatch_doc_id"), ("receipt_docs", "receipt_doc_id")):
        ...
        for r in rows:
            ...
            if kop:
                out[str(r["day"])] = out.get(str(r["day"]), 0) + kop
    return out


def _pallets_income_by_day(
    connection, *, date_from: str, date_to: str, client_id: str | None,
) -> dict[str, int]:
    """День (факт рейса) → доход от палет, копейки.

    Σ палет привязанных к рейсу отгрузок × цена палета клиента (effective-dated) на день
    ...
    for r in rows:
        ...
        if price:
            out[day] = out.get(day, 0) + price * pallets
    return out
...
def pnl_report(connection, *, date_from: str, date_to: str, client_id: str | None = None) -> dict:
    ...
    logistics = _empty()
    for day_iso, kop in _logistics_income_by_day(
        connection, date_from=df, date_to=dt, client_id=client_id
    ).items():
        i = idx.get(day_iso)
        if i is not None:
            logistics[i] += kop
    ...
    pallets = _empty()
    for day_iso, kop in _pallets_income_by_day(
        connection, date_from=df, date_to=dt, client_id=client_id
    ).items():
        i = idx.get(day_iso)
        if i is not None:
    ...
    income_series = [sum(col) for col in zip(*[s["series"] for s in income_sources])] if income_sources else _empty()
