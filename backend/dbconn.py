...
def barcode_variant_exists_sql(product_col: str, color_col: str, size_col: str) -> str:
    """EXISTS-фрагмент поиска позиции по штрих-коду товара; ждёт один параметр — сам код.

    Совпадение точное (сканер/буфер отдаёт код целиком) и с точностью до варианта:
    ШК принадлежит цвето-размеру, поэтому сужает выдачу до конкретной позиции,
    а не до всех вариантов товара.
    """
    return (
