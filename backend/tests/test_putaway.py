...
def test_full_putaway_flow(api, client_id, product, zones):
    ...
    # Товар в коробе на столе: он уже не «на хранении», но ещё не готов к отгрузке.
...
def test_finish_blocked_until_boxes_placed(api, client_id, product, zones):
    doc_id, _ = _create_task(api, client_id, product, qty=4)
    code = _new_box_code(api)
    _as(_WAREHOUSE)
...
def test_scan_limited_by_what_is_on_the_table(api, client_id, product, zones):
    """Жёсткий блок: в короб нельзя положить больше, чем передано на стол."""
    doc_id, _ = _create_task(api, client_id, product, qty=2)
    code = _new_box_code(api)
    _as(_WAREHOUSE)
    box_id = api.post(f"/shipments/{doc_id}/boxes", json={"code": code}).json()["id"]
...
def test_undo_scan_returns_item_to_table(api, client_id, product, zones):
    ...
    undo = api.post(
        ...
        json={"pack_entry_id": entries[0]["id"]},
    )
    assert undo.status_code == 200, undo.text
    assert undo.json()["items_qty"] == 0
    assert _bucket(client_id, product["product_id"], "boxed") == 0
    assert _bucket(client_id, product["product_id"], "packing") == 3
